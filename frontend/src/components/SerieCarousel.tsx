...
const SerieCarousel: React.FC<SerieCarouselProps> = ({ series }) => {
  ...
  const calculateItemsAndGap = () => {
    ...
    const gap = (windowWidth - maxItemsPerSlide * minItemWidth) / (maxItemsPerSlide - 1);
    const itemWidth = Math.max(minItemWidth, (windowWidth - gap * (maxItemsPerSlide - 1)) / maxItemsPerSlide);
    return {
          ...
          <div key={groupIndex} className="seriesHome__carouselSlide">
            {serieGroup.map((serie, serieIndex) => (
              <div
                ...
                style={{
                  width: `${itemWidth}px`,
                  marginRight: serieIndex < serieGroup.length - 1 ? `${gap}px` : "0",
                }}
              >
